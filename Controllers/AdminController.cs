        ...
        IBookBorrowingService bookBorrowingService)
    {
        ...
        public async Task<IActionResult> UpdatePaymentStatus(int orderId, bool isPaid)
        {
            try
            {
                ...
                {
                    throw new Exception("Nie udało się potwierdzić płatności.");
                }

                TempData["Success"] = $"Potwierdzono płatność za zamówienie #{orderId}. " +
                    $"Status został zmieniony na \"Opłacono\". Kwota: {updatedOrder.OrderTotal} zł";

                // Log the status change
                await _userActivityService.LogActivityAsync(
                    updatedOrder.UserId,
                    "PaymentStatusUpdate",
                    $"Admin potwierdził płatność za zamówienie #{orderId}"
                );
            }
